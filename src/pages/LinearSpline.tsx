...
import { Matrix } from '../components/Matrices.tsx'
import PointsInput from '../components/PointsInput.tsx'
import { SplinesPlot } from '../components/Plot.tsx'
...
export const LinearSpline = () => {
  ...
  type tracersType = [[number, number], string][]
  const [tracers, setTracers] = useState<tracersType>([])
  ...
  const post = () => {
    fetch(`http://localhost:3000/interpolation/${method}`, {
      ...
      res.json().then((json) => {
        console.log(json)
        if (json.success) {
          setTracers(json.data.tracers)
          setCoef(json.data.matrix)
          setShowPlot(true)
        } else {
          if (json.error) {
            setAlert(true)
        ...
        {showPlot &&
          <div className="flex flex-col items-center w-full gap-16 pt-10">
            <div className="flex flex-row w-full p-2 bg-[#2B2931] h-fit rounded-lg items-center">
              <SplinesPlot funcs={tracers} height={520} width={800} />
            </div>
            <div className="flex flex-col items-center  w-full gap-16">
              <div className="flex flex-row w-full gap-16">
                <Matrix matrix={coef} name={'Coefs'} />
              </div>
              {
